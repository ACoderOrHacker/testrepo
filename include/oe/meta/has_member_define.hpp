 ...
 *
 * Requires C++17 and you should import `oe.meta` first before using these
 * macros.
 *
 * @deprecated Using C++20 Concepts but not macro codes.
 *
...
#define OE_DEFINE_HAS_MEMBER(trait_name, member_name)                                              \
    template <class CheckType, class = void> struct trait_name : oe::false_type {};                \
    template <class CheckType>                                                                     \
    struct trait_name<CheckType, oe::void_t<decltype(oe::declval<CheckType>().member_name)>>       \
        : oe::true_type {};                                                                        \
    template <class CheckType>                                                                     \
    constexpr inline bool OE_PP_CONCAT(trait_name, _v) = trait_name<CheckType>::value;

/**
 * @def OE_DEFINE_HAS_TYPE
...
#define OE_DEFINE_HAS_TYPE(trait_name, type_name)                                                  \
    template <class CheckType, class = void> struct trait_name : oe::false_type {};                \
    template <class CheckType>                                                                     \
    struct trait_name<CheckType, oe::void_t<typename CheckType::type_name>> : oe::true_type {};    \
    template <class CheckType>                                                                     \
    constexpr inline bool OE_PP_CONCAT(trait_name, _v) = trait_name<CheckType>::value;
...
#define OE_DEFINE_HAS_MEMBER_WITH_TYPE(trait_name, member_name, type)                              \
    ...
    struct trait_name<                                                                             \
        CheckType,                                                                                 \
        oe::enable_if_t<std::is_same_v<decltype(oe::declval<CheckType>().member_name), type>>>     \
        : oe::true_type {};                                                                        \
    template <class CheckType>                                                                     \
    constexpr inline bool OE_PP_CONCAT(trait_name, _v) = trait_name<CheckType>::value;
...
#define OE_DEFINE_HAS_FUNCTION_MEMBER(trait_name, member_name, ret, ...)                           \
    template <class CheckType, class... Args> struct OE_PP_CONCAT(trait_name, _Details) {          \
      private:                                                                                     \
        template <class U>                                                                         \
        static auto check(int)                                                                     \
            -> decltype(oe::is_same_v<decltype(oe::declval<CheckType>().member_name(               \
                                          oe::declval<Args>()...)),                                \
                                      ret>,                                                        \
                        oe::true_type());                                                          \
        template <class U> static oe::false_type check(...);                                       \
                                                                                                   \
      public:                                                                                      \
        enum { value = oe::is_same<decltype(check<CheckType>(0)), oe::true_type>::value };         \
    };                                                                                             \
    template <class CheckType>                                                                     \
    using trait_name = OE_PP_CONCAT(trait_name, _Details)<CheckType, __VA_ARGS__>;                 \
    template <class CheckType>                                                                     \
    constexpr inline bool OE_PP_CONCAT(trait_name, _v) = trait_name<CheckType>::value;
